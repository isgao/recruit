 ...
 * @brief api入口类
 */
defined('API_PATH') OR define('API_PATH',dirname(__FILE__).'/api/');
/**
 * @class Api
 * @brief api处理类
 *
...
class Api
{
	...
	public static function run($apiName)
	{
		...
		{
			self::error($apiName,'Api_resource.php 里没 '.$apiName.' 这个参数');
		}
	...
	private static function query($queryInfo,$aguments)
	{
		...
		DB::enableQueryLog();
		//查询语句
		$tableObj = DB::table($queryInfo['name']);
		unset($queryInfo['name']);
		if (isset($queryInfo['type'])) 
		{
			$is_row = $queryInfo['type'];
			unset($queryInfo['type']);
		}

		foreach($queryInfo as $key => $val)
		{
			if (substr_count($key, 'join')) 
			{
				$key = 'join';
			}
			switch (substr_count($val,',')) {
				case '0':
					$tableObj->$key($val);
					break;
				case '1':
					list($parm1,$parm2) = explode(",", $val);
					$tableObj->$key($parm1,$parm2);
					break;
				case '2':
					list($parm1,$parm2,$parm3) = explode(",", $val);
					$tableObj->$key($parm1,$parm2,$parm3);
					break;
				case '3':
					list($parm1,$parm2,$parm3,$parm4) = explode(",", $val);
					$tableObj->$key($parm1,$parm2,$parm3,$parm4);
					break;
				default:
					$tableObj->$key(DB::raw($val));
					break;
			}
		}
		...
		{
			$dataResult = $tableObj->get();
			if(isset($is_row) && $is_row == 'row' && $dataResult)
			{
				return $dataResult[0];
			}
			return $dataResult;
